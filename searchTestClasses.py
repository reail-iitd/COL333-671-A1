...
class GraphSearch(SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        # print(self.print_push())
        if self.get_push():
            return list(self.successors[state])[::-1]
        else:
...
class GraphSearchTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict, result = []):
        ...
        # searchAgents = moduleDict['searchAgents'] # no need of search agents here
        ...
        if error != None:
            ...
            result.extend(["FAIL: error in getSolInfo",False,None,tok-tik])
            return False
        ...
        if solution in gold_solution and (not self.exactExpansionOrder or expanded_states in gold_expanded_states):
            ...
            result.extend(["PASS",True,len(expanded_states),tok-tik])
            return True
        else:
            grades.addMessage('FAIL: %s' % self.path)
            ...
            result.extend(["FAIL: solution doesn't match with GOLD",False,None,tok-tik])
            return False

    def writeSolution(self, moduleDict, filePath, result=[]):
        search = moduleDict['search']
        # searchAgents = moduleDict['searchAgents'] # no need of search agents here
        ...
        # search.REVERSE_PUSH = not search.REVERSE_PUSH
        handle.close()
        result.extend([None,True,None,None])
        return True
...
class PacmanSearchTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.newgoal = testDict.get('customgoal', None)
    ...
    def getSolInfo(self, search, searchAgents):
        ...
        if self.newgoal != None:
            coords = self.newgoal.split(',')
            problemOptions['goal'] = (int(coords[0]),int(coords[1]))
    ...
    def execute(self, grades, moduleDict, solutionDict, result=[]):
        ...
        searchAgents = moduleDict['searchAgents-TA']
        gold_solution = [str.split(solutionDict['solution']), str.split(solutionDict['rev_solution'])]
        ...
        if solution not in gold_solution:
            ...
            result.extend(["FAIL: doesn't match with GOLD",False,None,tok-tik])
            return False
        ...
        if expanded > self.leewayFactor * gold_expanded and expanded > gold_expanded + 1:
            ...
            result.extend(["FAIL: solution too costly",False,expanded,tok-tik])
            return False
...
class CornerProblemTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict, result = []):
        ...
        if type(solution) != type([]):
            ...
            result.extend(["FAIL: type(solution) != type([])",False,None,tok-tik])
            return False

        if len(missedCorners) != 0:
            grades.addMessage('FAIL: %s' % self.path)
            grades.addMessage('Corners missed: %s' % missedCorners)
            result.extend(["FAIL: {} corners missed".format(len(missedCorners)),False,None,tok-tik])
            return False
        ...
        if len(solution) != gold_length:
            ...
            result.extend(["FAIL: solution length not equal to gold length",False,None,tok-tik])
            return False

        grades.addMessage('PASS: %s' % self.path)
        grades.addMessage('\tpacman layout:\t\t%s' % self.layoutName)
        grades.addMessage('\tsolution length:\t\t%s' % len(solution))
        result.extend(["PASS",True,None,tok-tik])
        return True
...
class HeuristicTest(testClasses.TestCase):
    ...
    def setupProblem(self, searchAgents, searchAgents_ta):
        lay = layout.Layout([l.strip() for l in self.layoutText.split('\n')])
        gameState = pacman.GameState()
        gameState.initialize(lay, 0)
        problemClass = getattr(searchAgents_ta, self.searchProblemClassName)
    ...
    def execute(self, grades, moduleDict, solutionDict, result = []):
        ...
        problem, state, heuristic = self.setupProblem(searchAgents,searchAgents_ta)
        ...
        if not passed:
            ...
            result.extend(["FAIL: "+message,False,None,tok-tik])
            return False
        else:
            grades.addMessage('PASS: %s' % self.path)
            result.extend(["PASS",True,None,tok-tik])
    ...
    def writeSolution(self, moduleDict, filePath, result=[]):
        ...
        print(self.layoutText)
        problem, _, heuristic = self.setupProblem(searchAgents,searchAgents_ta)
...
class HeuristicGrade(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict, result = []):
        ...
        tok = time.time()
        expanded = problem.get_expanded()
        problem, _, heuristic = self.setupProblem(searchAgents,searchAgents_ta) # problem setup again
        if not checkSolution(problem, path):
            ...
            result.extend(["FAIL: Returned path is not a valid solution",False,None,tok-tik])
            return False
...
class ClosestDotTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict, result = []):
        ...
        if type(solution) != type([]):
            grades.addMessage('FAIL: %s' % self.path)
            grades.addMessage('\tThe result must be a list. (Instead, it is %s)' % type(solution))
            result.extend([None,False,None,None])
            return False
...
class CornerHeuristicSanity(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict, result = []):
        ...
        problem = searchAgents.CornersProblem(game_state) # problem declared again
        tik = 0
        tok = 0
        ...
        for succ in succs:
            ...
            if h0 - h1 > 1:
                ...
                result.extend(["FAIL: inconsistent heuristic",False,None,tok-tik])
                return False
        ...
        if heuristic_cost == 0:
            ...
            result.extend(["FAIL: must use non-trivial heuristic",False,None,tok-tik])
            return False
        # cornerAdmissible
        ...
        for i in range(0, len(heuristics) - 1):
            ...
            if h0 < 0 or h1 <0:
                ...
                result.extend(["FAIL: non-positive heuristic",False,None,tok-tik])
                return False
        # cornerGoalH
        if heuristics[len(heuristics) - 1] != 0:
            grades.addMessage('FAIL: heuristic non-zero at goal')
            result.extend(["FAIL: heuristic non-zero at goal",False,None,tok-tik])
            return False
